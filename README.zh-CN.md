...
最终体验是：本地能完成时尽量贴近设备直接执行，需要扩展算力时再接入远程执行网络。
...
如果你关注桌面 AI 产品、混合推理，或者分布式任务执行，这个仓库会直接把这套系统的核心形态展现在代码里。
...
- 提交产品、交互或实现层面的改进
- 研究一种比“纯云端”更务实的混合推理方案

## 开发说明

这个仓库的目标是可读、可公开、可移植。

它聚焦于 HEPH 的公开核心：桌面端流程、路由逻辑、前端行为，以及工作节点执行链路。对于理解产品形态而言，关键路径都已经在这里。
